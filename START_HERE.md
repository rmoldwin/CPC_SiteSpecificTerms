...
## Get this package into the new session (Google Drive direct download)

Paste this as the FIRST instruction in the new session — it fetches and unzips the
entire `icdo32_execution/` tree (verified: downloads a valid 1.2 MB zip, not an
HTML page):

```
Download and unzip this into the workspace, then follow icdo32_execution/START_HERE.md:
https://drive.google.com/uc?export=download&id=1V_aHv-RY3Yw0kmMPTrhAgILU65QiD18h
```

Because every prebuilt artifact (ceilings, candidates, 57 batches, seeded GI
results, pref_map) is already inside the zip, the new session can SKIP the
deterministic rebuild and go straight to the head_neck wave.
