...
- **Site labels use SPECIFIC 4-digit ICD-O-3.2 topography** (from `icdo32_topography.json`), never the 3-char parent group (e.g. C221 = "Intrahepatic bile duct", not the C22 "Liver" group).
- **Anatomic-correctness audit** of every "Yes" subset before release: each assigned site must be the tumor's site of ORIGIN, not a site of invasion or a naive substring/parent match.
- **Output:** README + "Site-Specific Terms" + "All Screened Terms" sheets, 13 columns, Calibri, teal header.

Anchor/regression check: **8140/3 Paneth cell carcinoma → "Yes"** (GI-only subset of a 186-site ceiling), sourced to Zhang et al., Diagnostic Pathology 2019 (PMC6323739). Label smoke test: **C221 renders as "Intrahepatic bile duct"**, not "Liver".
...
icdo32_pipeline/
  ...
  icdo32_topography.json         # 4-digit C-code -> specific site term map (labels)
  AUDIT_BRIEF.txt                # anatomic-correctness audit prompt for step-7 subagents
  scripts/
    01_build_ceilings.py         # SMVL -> per-code Valid-site ceilings; multi/single split
    01b_build_topography.py      # validate icdo32_topography.json covers every used C-code
    ...
    06_build_workbook.py         # final Excel (README + 2 sheets); 4-digit labels; optional CSVs
    07_site_correction_audit.py  # apply confirmed site corrections (ceiling+subset enforced; idempotent)
...
3. **Validate + merge + de-duplicate:** run script 4 across the accumulated `results/` — it rejects any site outside the ceiling, demotes bogus "Yes" (no proper subset / no source) to "Uncertain", merges the single-site auto-"No" terms, and collapses each `(code, term)` to exactly one row (a term bucketed into >1 organ system can appear in several result files). The dedup tie-break keeps the strongest decision, then for a "Yes" the smaller/more-specific sourced subset (site of origin), then the better source. NOTE: script 4 rebuilds `decisions.jsonl` from scratch and therefore wipes the step-7 site corrections — after any re-run of script 4 you MUST re-run `07 --apply` then script 6 (order: 4 → 7 → 6).
4. **Build workbook:** run script 6 → final Excel. Spot-check the Paneth anchor, the C221 label smoke test, and skim the validation report.
5. **Site-correction audit (before release):** extract all "Yes" rows with 4-digit labels, batch (~90/batch), and spawn ONE research subagent per batch with `AUDIT_BRIEF.txt` to flag `OK`/`WRONG_SITE`/`UNSURE` (source URL required for any `WRONG_SITE`). Fold confirmed fixes with `07_site_correction_audit.py --apply` (rejects any site outside the ceiling or not a strict subset), then re-run script 6. The 2026-07-09 audit found 6 real corrections out of 623 Yes (ceruminous 8420/3 → external ear; retinoinvasive melanoma 8720/3 → uvea; undifferentiated uterine sarcoma 8805/3 → corpus uteri); cholangiocarcinoma was a label-only fix.
6. **Refine loop:** if a wave's results look weak (too many "Uncertain", thin sources), edit `research_brief_template.md` or re-batch and re-run just that system.

### Version refresh (new ICD-O / SMVL / WHO Blue Book editions)
To reproduce the release on new inputs: (a) drop in the new `smvl.csv` and `master_terms.tsv` and, if the ICD-O topography axis changed, refresh `icdo32_topography.json` then run `01b_build_topography.py --validate` (must be 0 missing); (b) re-run steps 1→2→3→5, research the changed/new systems in waves, then 4→6→7→6; (c) re-check the anchor + C221 label and re-run the step-7 audit, updating WHO/NCCN source URLs for any "Yes" whose Blue Book classification changed.
