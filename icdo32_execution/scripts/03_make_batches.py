...
# Systems that are NEVER researched (locked rule 7). No batch is emitted for them.
# Step 2 already strips C76/C80 from effective ceilings; this is defense-in-depth so
# a term whose residual dominant site still maps here cannot leak into a batch.
EXCLUDE_SYSTEMS = {"ill_defined", "unknown_primary"}

# Systems already completed in an earlier session (folded in via seeded results).
# Terms whose dominant system is here are not re-batched. Override with env
# ICDO32_SKIP_SYSTEMS="" to force a full re-run.
SKIP_SYSTEMS = set(
    s for s in os.environ.get("ICDO32_SKIP_SYSTEMS", "digestive").split(",") if s
)
...
def main():
    ...
    with open(candfile) as f:
        for line in f:
            ...
            if sysname in EXCLUDE_SYSTEMS or sysname in SKIP_SYSTEMS:
                continue
